...
SimplePatternList gPatterns = {  defaultSetting, colorwave, solid, twinkle, twinkleShift, rainbow, strobe, twinkleTest };
...
void loop()
{
  ...
  EVERY_N_MILLIS_I( hueTimer, 25 ) { // timer name, inital hue speed == 25
    ...
    nblendPaletteTowardPalette( gCurrentPalette, gTargetPalette, 8); // for colorwave
  }
  EVERY_N_SECONDS( 20 ) { // Color palette timer
    gCurrentPaletteNumber = random8( gGradientPaletteCount);
    gTargetPalette = gGradientPalettes[ gCurrentPaletteNumber ];
  }
  EVERY_N_SECONDS( 60 ) { // mode cycle timer
    nextPattern();  
  }
...
void bpm()
{
  ...
  uint8_t BeatsPerMinute = map(hueUpdate, 40, 1, 30, 0);
...
void trailingDot( uint8_t sinSpeed )
{
  // a colored dot sweeping back and forth, with fading trails // could be useful w/ other configurations
  fadeToBlackBy( leds, NUM_LEDS, 10);
  int pos = beatsin16( sinSpeed, 0, NUM_LEDS - 1 ); // speed, phase offset, range
